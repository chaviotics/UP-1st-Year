...
int i,j,Field[N][M],x,y,Gy,Head,Tail,Game,Cursed_Apple,a,b,var,dir,score,HighScore,speed,level;
...
/*
    ...
    level holds the difficulty of the game.
...
void main(){
    ...
    game_start(); // This is the starting screen of the game
...
void game_start(){
    ...
    printf("==========================================\n");
    printf("||  Welcome to the REVERSED Snake Game! ||\n");
    printf("==========================================\n\n");
...
void game_story(){
    ...
    printf("The story: \n");
    printf("\n");
    printf("We all know the classic snake game.\n"); \
    printf("There is the snake, and every time it eats the apple,\n");
    printf("the snake grows in length.\n");
    printf("\n");
    printf("In this game, the snake has been cursed\n");
    printf("and will get smaller every time it eats the cursed apple.\n");
    printf("The objective of the game is eat as many apples \n");
    printf("until the snake is gone. \n");
    printf("\n");
    printf("Good luck!\n");
    printf("\n");
    printf("         Press ENTER to continue...\n\n");
...
void game_mechanics(){
    ...
    printf("The Mechanics: \n");
    printf("\n");
    printf("- Use the WASD keys to move the snake.\n"); \
    printf("- Every time the snake eats the 'cursed apple', it gets smaller.\n");
    printf("- Since the snake is getter smaller, it will be faster as it loses its body weight.\n");
    printf("- As the snake eats the 'cursed apple', its body and tail gets left behind.\n");
    printf("- For added challenge, you cannot eat your own 'sliced' body. (The snake hates its own taste)\n");
    printf("\n");
    printf("Think of the classic snake game, \n");
    printf("but REVERSED. \n");
    printf("\n");
    printf("         Press ENTER to continue...\n\n");
...
void game_difficulty(){
    ...
    printf("Chooose your difficulty (choose the number): ");
    printf("\n1 - EASY");
    printf("\n2 - MEDIUM");
    printf("\n3 - HARD");
    printf("\n");
    printf("Note: Choosing a difficulty above 3 makes it harder.");
    printf("\n");
    printf("\n");
    printf("Your difficulty: ");

    scanf("%d",&level); // After the short intro of the story, this is where the player chooses his/her difficulty.
    ...
    while(1){
        ...
        if(var == 13){
            system("Cls");
            snake_initialization(); // This initializes the snake
            break;
        }
    }
    system("Cls");
...
void movement(){// This function is for controlling the movement of the snake. This takes input from the user.
    ...
    if(dir == 'd'){ // This moves the snake to the RIGHT.
        ...
        if(Head <= Tail){GameOver();} // If the position of the Head and the Tail are the, it will go the the GameOver() function. This is the part where the head is the only part left
        ...
        if(Field[x][y] ==  -1){ // This generates a new cursed_apple randomly whenever it is "eaten" by the snake.
            ...
            Tail += 2; // This also translates to "Tail = Tail + 2". This decreases the length of the snake by increasing the value of the tail, whenever the snake "eats" the cursed apple.
        }
        Head++; // moves the head
    ...
    if(dir == 'w'){ // This moves the snake to the TOP.
        ...
        if(Field[x][y] != 0 && Field[x][y] != -1){GameOver();}
        if(Head <= Tail){GameOver();}
        if(x == 0){x = N - 1;} // From top, it will go to the bottom. -1 so that it will be better and more realistic.
        if(Field[x][y] ==  -1){
            Cursed_Apple = 0;
            score += 5;
            Tail += 2;
...
void GameOver(){ // This function makes the game over - this is when the snake eats its own body or eats the cursed_apple.
    ...
    while(1){
        ...
        if(var == 13){ // 13 is the ASCII key for ENTER.
            ...
            game_start(); // initialize back the game
